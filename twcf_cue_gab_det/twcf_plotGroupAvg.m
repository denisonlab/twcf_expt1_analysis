...
% figDir = '/Users/karen/Dropbox/github/TWCF_FOHO/twcf_expt1_analysis_BU/twcf_cue_gab_det/figs'; % lab temp
if ~exist(figDir, 'dir')
    mkdir(figDir)
end
...
%% Plot labels 
cVarNames = {'C1','C2'}; 
cVarShortNames = {'saw stimulus','saw feature'}; % {'saw grating','saw tilt'}; 
cVarLabelNames = {'p("saw grating")','p("saw tilt")','p("saw grating not tilt")'};
...
for iS = 1:numel(subjectIDs)
    val_x(iS,:)      = a(iS).uniqueContrasts_postcue;
    val_logx(iS,:)   = a(iS).log10_uniqueContrasts_postcue;
    val_C(iS,:,:)    = a(iS).(cFieldNames{iC}).yMean;
    val_A(iS,:,:)    = a(iS).A.yMean;

    val_overall(iS,:,:) = a(iS).(cFieldNames{1}).yMean;
    val_feature(iS,:,:) = a(iS).(cFieldNames{2}).yMean;
    for iL = 1:nStrengths
        for iAtt = 1:3
...
figure
set(gcf,'Position',p.size.rect) % [0 0 380 280]
...
a_A = twcf_plotFitGroup(var,varName,varShortName,group_a,p,taskType,s,val_A,0,a);
...
%% 
% disp('NR fit')
% paramsFittedMat.nakarushton = a_A.A.paramsFitted;  
% 
% paramsFittedMat.weibull = a_A.A.paramsFitted; 
...
p.iPlot = p.iPlot+1; 
% stimIdx = 2:8; % do not plot contrast 0 
...
a_C.(cVarNames{iC}) = twcf_plotFitGroup(var,varName,varShortName,group_a,p,taskType,s,val_C,0,a);
...
if plotFigAbsent
    ...
    varShortName = cVarShortNames{iC};
    taskType = 'detection';
    a_C_absent.(cVarNames{iC}) = twcf_plotFitGroupAbsent(var,varName,varShortName,group_a,p,taskType,s,val_C_absent);
end
...
%% Subjective - saw stimulus and saw feature overlaid, saw stimulus/saw feature
...
if iC==1
    ...
    subplot(5,2,[5 6 7 8 9 10])
    % Plot overall visibility
    var_overall = squeeze(mean(val_overall,1));
    varName = cVarNames{1};
    varShortName = cVarShortNames{1};
    taskType = 'detection';
    group_a.outOfNum = squeeze(a(1).(varName).OutOfNum)';

    twcf_plotFitGroup_objectiveFeature(var_overall,varName,varShortName,group_a,p,taskType,s,val_overall,0,a,'overall');

    % Plot feature visibility
    var_feature = squeeze(mean(val_feature,1));
    varName = cVarNames{1};
    varShortName = cVarShortNames{1};
    taskType = 'detection';
    group_a.outOfNum = squeeze(a(1).(varName).OutOfNum)';

    twcf_plotFitGroup_objectiveFeature(var_feature,varName,varShortName,group_a,p,taskType,s,val_feature,0,a,'feature');
    
    ylabel('Subjective report')

    subplot(5,2,[1 2 3 4])
    hold on
    figureStyle
    for iAtt=1:3
        pl = plot(group_a.uniqueContrasts_postcue, var_feature(:,iAtt)./var_overall(:,iAtt));
        pl.LineWidth = 1.5; 
        pl.Color = p.style.attColors(iAtt,:);
    end
    plot(group_a.uniqueContrasts_postcue, var_feature(:,3)./var_overall(:,3) - var_feature(:,1)./var_overall(:,1),...
        'LineWidth',1.5,'Color',[199,193,205]/255); % p.style.attColors(4,:));
    xlim(a_A.A.xl)
    xticks('')
    yline(0)
    ylabel(sprintf('p(saw feature/\nsaw stimulus)'))
    % overlay these and highlight the intersection 
    % add points to the difference fig 

    if s.saveFigs
        figTitle = sprintf('group_n%d_%s_overallFeature',numel(subjectIDs),s.site);
        export_fig(gcf,sprintf('%s/%s.%s', figDir, figTitle, s.figType),'-transparent','-p10')
    end
end
...
% plot stimulus
var = squeeze(mean(val_C_criterion_stimulus,1,'omitnan'));
varName = cVarNames{iC};
cVarShortNames = {'saw grating','saw orientation'};
...
%% Det d' (stimulus and feature) vs. discrimination d'
% double error bar vertical line 
% Calculate discirmination d' for group (30 x 8 x 3 (att) x 1 (feature only)
sdtType = 'EV'; 
for iS=1:30
...
switch sdtType 
    case 'EV'
        taskType = 'dprime_det_dis';
    case 'UV'
        taskType = 'dprime_det_dis_UV';
end
twcf_plotFitGroup(var,varName,varShortName,group_a,p,taskType,s,val_C_dprime);
...
subplot 111 
subplot('Position', [0.4 0.2074 0.45 0.70]) % p.size.AUC
hold on
figureStyle
for iAtt = 1:numel(varA.attHeaders)
    ...
    bar(x(iAtt),y(iAtt),'FaceColor',p.style.attColorsMuted(iAtt,:),'EdgeColor','none')
