...
% [a, PSI] = twcf_behavAnalysis(dataAll,subjectIDs,subjectID,s)
% Behavioral analysis for cue tex det fmri (using runBehav.m) 
% 
% Inputs 
...
% C
cVarNames      = {'sawFigure','sawShape'}; 
cFieldNames    = {'C1','C2'}; % ,'CX'}; 
cVarLabelNames = {'p(saw stimulus)','p(saw feature)'}; % 'p(saw figure not shape)'}; 
...
% Attention 
a.attConds   = [-1 1]; % unique(data.cueValidity(~isnan(data.cueValidity))); % -1 --> invalid, 1 --> valid, 0 --> neutral
a.attHeaders = {'invalid','valid'};
...
a.uniqueLineLengths_inDeg_postcue = unique(data.lineLength_inDeg_postcue(~isnan(data.lineLength_inDeg_postcue))); % 5 stimulus strengths 
...
a.log10_uniqueLineLengths_inDeg_postcue = log10(unique(data.lineLength_inDeg_postcue(~isnan(data.lineLength_inDeg_postcue))));
...
nTrials.all = numel(data.resp(~isnan(data.resp))); 

% number of trials for accuracy
nTrials.invalid = 0; nTrials.valid = 0; 
for i = 1:numel(a.uniqueLineLengths_inDeg_postcue)
    nTrials.invalid = nTrials.invalid + numel(a.correctDis{i,1});
    nTrials.valid = nTrials.valid + numel(a.correctDis{i,2});
end
...
for iC = 1:numel(cFieldNames)
    for iL = 1:numel(a.uniqueLineLengths_inDeg_postcue)
        for iAtt = 1:numel(a.attHeaders)
            ...
            [sdt.(condName).dprimeDetect(iL,iAtt), sdt.(condName).criterionDetect(iL,iAtt)] = rd_dprime2(nh,nfa,nsignal,nnoise);
...
for iL = 1:numel(a.uniqueLineLengths_inDeg_postcue)
    for iAtt = 1:numel(a.attHeaders)
        ...
        [sdt.(condName).dprimeDis(iL,iAtt), sdt.(condName).criterionDis(iL,iAtt)] = rd_dprime2(nh,nfa,nsignal,nnoise);
...
sgtitle(sprintf('expt: %s | site: %s \n subject: %s | %d blocks', s.exptName, s.site, subjectID, max(data.i_block)))
p.iPlot = 0; 
...
sgtitle(sprintf('Fitted params\nexpt: %s | site: %s \n subject: %s', s.exptName, s.site, subjectID))
p.iPlot = 1;
...
figure
set(gcf,'Position',[0 0 350 450]) % p.style.figHeight
sgtitle(sprintf('CvA \nexpt: %s | site: %s \n subject: %s', s.exptName, s.site, subjectID))
p.iPlot = 1; 
