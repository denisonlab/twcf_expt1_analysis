...
xlabel('A')
ylabel(sprintf('%s',varC.varName))
attIdx = [1 2]; % only invalid and valid for fmri experiments

%% Plot CvA by attention 
for iAtt = attIdx % 1:numel(varC.attHeaders) 
    % Plot datapoints
...
minC = max(rangeC(attIdx,1));
maxC = min(rangeC(attIdx,2));
minA = max(rangeA(attIdx,1));
maxA = min(rangeA(attIdx,2));
