...
%% Get stim strengths
fName = 'uniqueLineLengths_inDeg_postcue'; 

for iS = 1:numel(PSI)
    for iL = 1:7
        g.(fName).val(iS,iL) = PSI(iS).(cFieldNames{iC}).(fName)(iL); 
    end
end
...
for iAtt = 1:numel(PSI(1).(cFieldNames{iC}).attConds)
    for iS = 1:numel(PSI) % subjects
        % detection sdt
        g.dprimeDetect.val(iS,:,iAtt) = a(iS).sdt.(cNames{iC}).dprimeDetect(:,iAtt);
        g.criterionDetect.val(iS,:,iAtt) = a(iS).sdt.(cNames{iC}).criterionDetect(:,iAtt);
        g.det_pfa.val(iS,:,iAtt) = a(iS).sdt.(cNames{iC}).det_pfa(:,iAtt);
        % discrimination sdt
...
for iG = 1:numel(gFields)
    y = g.(gFields{iG}).val; % subjects x line length x att 
    x = 1:7; 
