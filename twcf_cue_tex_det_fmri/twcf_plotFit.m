...
NumPos = []; OutOfNum = [];
for iAtt = [1 2] % numel(a.attHeaders)
...
switch p.fit.PFtype 
    case 'gumbel'
        xlabel('log_{10} (line length) at cued loc (°)')
        xlim([stimLevels(1)-(p.style.xBuffer/10) stimLevels(end)+p.style.xBuffer])
    case 'weibull'
        set(gca,'Xscale','log')
        xlabel('Line length at cued loc (°)')
        xlim([stimLevels(1)-(p.style.xBuffer/10) stimLevels(end)+(p.style.xBuffer/4)])
end
xticks(stimLevels)
xl = get(gca, 'XLim');
