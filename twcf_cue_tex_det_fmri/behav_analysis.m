function [a,PSI] = behav_analysis(dataAll,subjectIDs,subjectID,s)
...
s.exptName = 'cue tex det fmri pilot1'; 
s.site = 'BU'; 
subjectID = 'S0003'; 

%% Setup 
% set to analysis folder 
addpath(genpath(pwd))

% Get figure and psychometric function fitting parameters 
p = twcf_analysisParams; 

%% Load data (manually load for fMRI pilot) 
% % * * * change here * * *
...
a.attHeaders = {'neutral'}; % {'invalid','neutral','valid'};
...
idx.sawFigure(idxCase)           = NaN;
idx.sawFigureNS(idxCase)         = NaN;
idx.sawShape(idxCase)            = NaN;
...
for i = 1:numel(a.uniqueLineLengths_inDeg_postcue)
    ...
    for iAtt = 1:numel(a.attHeaders)
        ...
        % Detection 
        a.sawFigure{i,iAtt}       = idx.sawFigure(idx.line & idx.good & idx.att); %  & idx.stimPresent
        a.sawFigureNS{i,iAtt}     = idx.sawFigureNS(idx.line & idx.good & idx.att); % & idx.stimPresent
        a.sawShape{i,iAtt}        = idx.sawShape(idx.line & idx.good & idx.att); % & idx.stimPresent 
...
% Trial-by-trial SDT categories 
% hit = data.i_trial(data.respDet==1 & data.stimID_postcue>0); 
% miss = data.i_trial(data.respDet==0 & data.stimID_postcue>0); 
% reject = data.i_trial(data.respDet==0 & data.stimID_postcue==0); 
% fa = data.i_trial(data.respDet==1 & data.stimID_postcue==0); 
