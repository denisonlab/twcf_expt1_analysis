...
for iAtt = 1:numel(PSI(1).(cFieldNames{iC}).attConds)
    for iS = 1:numel(PSI)
        for iG = 1:numel(gFieldNames)
            g(iC).(gFieldNames{iG}).val(iS,iAtt) = PSI(iS).(cFieldNames{iC}).(gFieldNames{iG})(iAtt);
        end
        g(iC).pCorrect.val(iS,:,iAtt) = PSI(iS).(cFieldNames{iC}).A.yMean(:,iAtt); 
        g(iC).pStronger.val(iS,:,iAtt) = PSI(iS).(cFieldNames{iC}).(cFieldNames{iC}).yMean(:,iAtt); 
    end
end

%% Plot group vars by att 
for iG = 1:numel(gFieldNames)
    y = g(iC).(gFieldNames{iG}).val;
    x = [1 2 3];
    ...
    g(iC).(gFieldNames{iG}).h = h; 
    g(iC).(gFieldNames{iG}).pVal = pVal; 
    g(iC).(gFieldNames{iG}).ci = ci; 
    g(iC).(gFieldNames{iG}).stats = stats; 
    g(iC).(gFieldNames{iG}).h = h; 
...
gFields = {'pCorrect','pStronger'};
for iG = 1:numel(gFields)
    y = g(iC).(gFields{iG}).val; % subjects x line length x att 
    x = 1:7; 
    ...
    if s.plotFigs
        ...
        xticks(1:7)
        xlabel('Stimulus strength (ordinal)')
        switch gFields{iG}
            case 'pCorrect'
                ylabel('p(correct)')
            case 'pStronger'
                ylabel(sprintf('p(test stronger) %s', cFieldNames{iC}))
        end
    end
    ...
    for iL = 1:7
        ...
        invalid = y(:,iL,1);
        [h pVal ci stats] = ttest(invalid,valid);
        g(iC).(gFields{iG}).h(iL) = h;
        g(iC).(gFields{iG}).pVal(iL) = pVal;
        g(iC).(gFields{iG}).ci(iL,:) = ci;
        g(iC).(gFields{iG}).stats(iL) = stats;
    end
...
if s.exportcsv
    tableHeaders = {'site', 'att','subjectID',...
        ...
        'alphaC','betaC','gammaC','lambdaC',...
        'alphaA','betaA','gammaA','lambdaA',...
        'alphaR','betaR','gammaR','lambdaR'}; % relative params 
    count = 1;
    for iAtt = 1:numel(PSI(1).(cFieldNames{iC}).attConds)
        for iS = 1:numel(PSI)
    ...
    csvName = sprintf('%s_%s_%s_group',...
        s.exptFolder,s.site,cFieldNames{iC}); % saves to csv folder 
...
if s.exportcsv
    tableHeaders = {'site', 'att','subjectID',...
        'lineLength','pCorrect','pStronger'};
    count = 1;
    for iAtt = 1:numel(PSI(1).(cFieldNames{iC}).attConds)
        for iS = 1:numel(PSI)
            for iL = 1:7
                for iG = 1:numel(gFields)
                    V.lineLength(count) = iL;
                    V.(gFields{iG})(count) = g(iC).(gFields{iG}).val(iS,iL,iAtt);
...
%% Combine multiple saved figures: 
