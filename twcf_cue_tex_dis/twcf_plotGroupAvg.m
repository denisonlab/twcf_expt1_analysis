...
cVarLabelNames = {'p(test stronger)'}; % {'p(test stronger than ref)'}
...
for iS = 1:numel(subjectIDs)
    val_x(iS,:)      = a(iS).lineLengths_inDeg_postcue;
    val_logx(iS,:)   = a(iS).log10_lineLengths_inDeg_postcue;
...
if s.saveFigs
    ...
    % figPath = sprintf('%s/%s.%s', figDir, figTitle, s.figType); 
...
%% Subjective C p(test stronger than reference) 
figure
set(gcf,'Position',[0 0 380 280])
% sgtitle(sprintf('expt: %s | site: %s', s.exptName, s.site))
p.iPlot = 0; 
...
if annotateStats
    yl = ylim; 
    xl = xlim; 
    txt = twcf_annotateStats(2,max(yl)*p.sigStarMainEffect,'***'); % main effect
    
    % I vs V
    yP = 0.75; 
    txt = twcf_annotateStats(2,max(yl)*yP,'***'); 
    twcf_drawBracket(1,3,yP)

    % I vs N 
    yP = 0.62; 
    txt = twcf_annotateStats(1.5,max(yl)*yP,'***'); 
    twcf_drawBracket(1,2,yP)

    % N vs V 
    yP = 0.38; 
    txt = twcf_annotateStats(2.5,max(yl)*yP,'***'); 
...
for iSite = 1:2
    for iAtt = 1:numel(varA.attHeaders)
        ...
        if iSite==1 % BU 
            x = xPos; 
            yIdx = find(contains(sites,'BU')); % BU edit to dynamic index based on sites
        elseif iSite==2 % UCI 
            x = xPos+4;
            yIdx = find(contains(sites,'UCI')); % BU edit to dynamic index based on sites
        end
...
varName = cVarNames{iC};
cVarShortNames = {'test stronger'};
varShortName = sprintf('Reference comparison d''');
taskType = 'dprime';
...
varName = cVarNames{iC};
cVarShortNames = {'test stronger'};
varShortName = sprintf('Reference comparison criterion');
taskType = 'criterion';

referenceAnnotation = 1; 
a_C_dprime.(cVarNames{iC}) = twcf_plotFitGroup(var,varName,varShortName,group_a,p,taskType,s,val_C_criterion,referenceAnnotation);
