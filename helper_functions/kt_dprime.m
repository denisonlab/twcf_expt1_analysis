function [dprime, criterion, d_a, c_a] = kt_dprime(nh,nfa,nsignal,nnoise,loglinear,s)
...
% 
% If desire to correct for cells with 0 or full counts (which will return NaN or Inf measures), 
% set loglinear to 1. Defaults to no correction. 
% 
% If unequal variance, and desire to calculate d_a and c_a (according to Hautus, Macmillan, Creelman 2022 eq 3.5),
% additional inputs are required:  
% s (slope) of the receiver operator curve
...
if loglinear
    % if h==0 || h==1 || fa==0 || fa==1 % apply whether or not edge case
        nh = nh + 0.5;
        nfa = nfa + 0.5;
        nnoise = nnoise + 1;
        nsignal = nsignal + 1;
