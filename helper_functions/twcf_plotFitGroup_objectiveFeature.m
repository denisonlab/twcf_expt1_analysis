function a = twcf_plotFitGroup_objectiveFeature(var,varName,varShortName,a,p,taskType,s,varSubjects,referenceAnnotation,subjectA,visibilityMeasure)
...
switch taskType
    case 'detection'
        ...
        elseif contains(varName,'C') % consciousness
            ylabel(sprintf('Visibility report'),'FontSize',p.style.textAxisSize)
        end
    case 'discrimination'
        ylims = [0.4 1];
        yticks(0:0.25:1)
...
for iAtt = 1:nAtt
    if strcmp(s.exptShortName,'twcf_cue_gab_det') && (strcmp(taskType,'dprime') || strcmp(taskType,'criterion') || strcmp(taskType,'criterion_diff'))
    ...
    switch taskType
        case {'detection','discrimination'}
            ...
            l(iAtt) = plot(stimLevelsFine,fit,'Color',p.style.attColors(iAtt,:),'linewidth',lineWidth,'linestyle',lineStyle);
    ...
    switch taskType
        case {'dprime','criterion','comparison','RT'}
            % if strcmp(p.exptShortName,'cue_tex_det')
    ...
    for iL = stimIdx
        ...
        sc = scatter(x,y,p.style.sz*0.4,'MarkerFaceColor',color,'MarkerFaceAlpha',1,'MarkerEdgeColor','w','LineWidth',lineWidth*0.75);
        ...
        if plotError
            ...
            switch taskType
                case 'criterion_diff'
                    errorbar(x,y,se(iL),'Color',p.style.attColorsMutedLight(4,:),'CapSize',0,'LineWidth',lineWidth)
                otherwise
                    errorbar(x,y,se(iL),'Color',p.style.attColorsMutedLight(iAtt,:),'CapSize',0,'LineWidth',lineWidth)
            end
        end
        ...
        if plotErrorX
            ...
            errorbar(x,y,se,'horizontal','Color',p.style.attColorsMutedLight(iAtt,:),'CapSize',0,'LineWidth',lineWidth)
        end
...
if plotConditionedOnPrecue
    ...
    for iAtt = 1:numel(a.attHeaders)
        for iL = stimIdx
            ...
            sc = scatter(x,y,p.style.sz,'MarkerFaceColor',p.style.attColors(iAtt,:),'MarkerFaceAlpha',1,'MarkerEdgeColor','w','LineWidth',lineWidth);
                sc.Marker = '+';
...
if referenceAnnotation
    xline(stimLevels(4),'LineWidth',1)
    yl = ylim;
    xl = xlim;
